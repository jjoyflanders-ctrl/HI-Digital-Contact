...
const els = {
  ...
  phoneQuickMob: document.getElementById("phoneQuickMob"),
  emailQuickMob: document.getElementById("emailQuickMob"),
  webQuickMob: document.getElementById("webQuickMob"),
  mobShareModal: document.getElementById("mobShareModal"),
  mobNativeShareBtn: document.getElementById("mobNativeShareBtn"),
  mobCopyLinkBtn: document.getElementById("mobCopyLinkBtn"),
  mobTextLinkBtn: document.getElementById("mobTextLinkBtn"),
  mobEmailLinkBtn: document.getElementById("mobEmailLinkBtn"),
...
function renderEmployee(emp) {
  ...
  setLink(els.phoneQuickMob, emp.phone ? `tel:${emp.phone}` : "#");
  setLink(els.emailQuickMob, emp.email ? `mailto:${emp.email}` : "#");
  setLink(els.webQuickMob, emp.website || "#");
...
function wireMobileControls() {
  ...
  els.shareBtnMob?.addEventListener("click", async () => {
    ...
    // Prefer native share on mobile (AirDrop / Messages / Mail / Notes, etc.)
    const ok = await nativeShare({
      title: current.name,
      text: `${current.name} — ${current.title}`,
      url: shareUrl,
    ...
    // Fallback: open a QR-FREE share sheet (copy/text/email)
    if (els.mobShareModal?.showModal) {
      els.mobShareModal.showModal();
    } else {
  ...
  // Mobile share fallback modal (QR-free)
  els.mobNativeShareBtn?.addEventListener("click", async () => {
    if (!current) return;
    const shareUrl = publicProfileUrl(current.id);
    const ok = await nativeShare({
      title: current.name,
    ...
    if (ok) els.mobShareModal?.close?.();
  });

  els.mobCopyLinkBtn?.addEventListener("click", async () => {
    if (!current) return;
    await copyToClipboard(publicProfileUrl(current.id));
    toast("Link copied.");
  });

  els.mobTextLinkBtn?.addEventListener("click", async () => {
    if (!current) return;
    window.location.href = `sms:&body=${encodeURIComponent(publicProfileUrl(current.id))}`;
  });

  els.mobEmailLinkBtn?.addEventListener("click", async () => {
    if (!current) return;
    window.location.href = `mailto:?subject=${encodeURIComponent(current.name || "Contact")}&body=${encodeURIComponent(publicProfileUrl(current.id))}`;
  });
